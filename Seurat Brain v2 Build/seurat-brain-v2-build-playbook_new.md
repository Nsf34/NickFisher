...
7. **⚠️ WRITE LOCATION — MANDATORY CHECK.** All file paths in wave prompts are RELATIVE to seurat-brain-v2. Before writing ANY files, verify your working directory ends with `seurat-brain-v2`. If it does not, STOP and tell Nick. You will read SOURCE files from the old brain (`seurat-brain`) using absolute paths — do NOT confuse this with your write destination. Never write files to the old brain (`seurat-brain`). Every file you create or edit goes into `seurat-brain-v2`.
...
- Project subfolders: detect numbered folders (regex: ^\d{2}\s+.+) like "01 Project Name"
  ...
  - Milestone coverage: which phases have versioned files (indicates completion)
  ...
  - Files with version numbers: extract version + initials chain from filename
    VERSION PARSING NOTE: Versions in Seurat filenames are loose conventions. Handle all of these:
    - "v1.0", "v0.3", "v2.1", "v.82" (missing leading zero = v0.82)
    - "v1.0 TMB NF" (version + initials chain)
    - No version at all (many files) — treat as unversioned, still extractable
    Regex pattern: v\.?(\d+\.?\d*) — covers v1.0, v.82, v0.3, v12, etc.
  - **Version grouping:** Group files by base name (ignoring version + initials). For each group, identify the HIGHEST version. This is what gets extracted — not v1.0 specifically, but whatever the latest version is.
...
Table: files
  ...
  - version TEXT (nullable — extracted from filename, e.g., "1.0", "0.82", "2.1")
  - version_group TEXT (nullable — base filename with version/initials stripped, for grouping)
  - is_highest_version BOOLEAN (nullable — true if this is the latest version in its group)
  - confidence TEXT (nullable — 'final' if v1.0+, 'draft' if v0.x only, 'unversioned' if no version)
...
VERSION SELECTION (query SQLite before downloading):
- **Highest-version-wins:** Group files by base name (strip version numbers + initials). For each group, extract ONLY the highest version. Mark lower versions as "skipped: superseded-by-vX.X"
- **Unversioned files:** Always include — no version ≠ unimportant. Many critical files (data sheets, briefs, plans) have no version number.
- **Draft-only files (highest version is v0.x):** Extract, but tag with `confidence: draft` in the extract header. Synthesis sessions should weight these lower than v1.0+ files.
- **Version parsing:** Handle all formats: "v1.0", "v0.3", "v2.1", "v.82" (= v0.82), "v12". Regex: v\.?(\d+\.?\d*)
...
version: [if detected from filename — e.g., "1.0", "0.82", "2.1"]
is_highest_version: [true/false — was this the latest version in its file group?]
confidence: [final (v1.0+) | draft (v0.x only) | unversioned (no version detected)]
...
CLI:
  ...
  python digestion_engine.py extract --latest-only                    Only highest version per file group (DEFAULT — on by default)
  python digestion_engine.py extract --all-versions                   Extract all versions, not just highest
  python digestion_engine.py extract --finals-only                    Only v1.0+ files (skip drafts entirely)
...
After writing, test:
python digestion_engine.py extract --client "Henkel" --latest-only --limit 10
Report: files downloaded + extracted, success/fail counts, content depth distribution, version selection decisions (how many files were superseded by newer versions), sample extract quality (paste first 50 lines of one rich extract), temp files properly cleaned up, disk usage.
```
...
2. EXTRACT TEST:
   python digestion_engine.py extract --client "Henkel" --latest-only --limit 5
...
3. VERSION SELECTION TEST:
   - Find a Henkel file that exists in multiple versions (e.g., v0.1, v0.3, v1.0, v2.0)
   - Verify: only the HIGHEST version was extracted, lower versions marked "skipped: superseded-by-vX.X"
   - Find an unversioned file — verify it was included (not skipped)
   - If any files are draft-only (highest is v0.x), verify they're tagged `confidence: draft`
...
5. STATUS TEST:
   ...
   - Verify: shows correct counts for pending/extracted/skipped/superseded

6. FULL SCAN TEST (if time allows):
...
"Source: extracts/henkel/01-hair-care/WS1-Deck-v2.0.txt, slides 12-15" (always cite the actual version extracted — could be v0.3, v1.0, v2.0, etc.)
...
0. Check digestion engine status:
   ...
   Verify: all priority clients show extraction_status = 'extracted' for their highest-version files. Check that superseded versions were correctly skipped and draft-only files are tagged `confidence: draft`.
